...
-- 1. Ensure core lookup/profile tables exist for fresh databases
...
CREATE UNIQUE INDEX IF NOT EXISTS user_auth0_sub_idx
  ON "USER" (auth0_sub);
...
-- 2. Align columns with the live schema shape
...
ALTER TABLE locations ADD COLUMN IF NOT EXISTS state VARCHAR(255);
ALTER TABLE locations ADD COLUMN IF NOT EXISTS country VARCHAR(255);
ALTER TABLE locations ADD COLUMN IF NOT EXISTS popularity INTEGER;
ALTER TABLE locations ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW();
ALTER TABLE "INSTITUTION" ADD COLUMN IF NOT EXISTS name VARCHAR(255);

-- 3. Seed locations table with 1000 US cities (ranked by population)
...
-- 4. Seed INSTITUTION table with US universities (skip duplicates)
